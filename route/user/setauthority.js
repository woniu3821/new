...
module.exports = async (ctx, next) => {
  ...
  try {
    await dbUtils.query(
      `UPDATE user_table SET frames="${body.frames}" WHERE uid IN(${inuid})`
    );
    await dbUtils.query(`UPDATE authority_table SET permission=CASE fid
        ...
        WHEN 16 THEN "${sub["16"].permission}"
        WHEN 17 THEN "${sub["17"].permission}"
        WHEN 18 THEN "${sub["18"].permission}"
        END,
        orders=CASE fid
        ...
        WHEN 16 THEN ${sub["16"].order}
        WHEN 17 THEN ${sub["17"].order}
        WHEN 18 THEN ${sub["18"].order}
        END
        WHERE uid IN(${inuid})`);
    ctx.body = {
      type: "success",
